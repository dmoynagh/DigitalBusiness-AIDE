AIDE Documentation | WIP | AIDE_Documentation_WIP@v4 | 2026-09-14
...
AIDE needs a mechanism for submitting learnings, additions, changes and new functionality from work sessions into an AIDE development queue. The charter's frictionless capture principle (2026-09-14) governs the design.

**The problem it solves.** Knowledge, findings and improvement opportunities are identified at the point of doing work — not during AIDE development sessions. Without a low-friction path from work to AIDE development, items are lost, deferred indefinitely, or require the user to stop work and switch context to capture them.

**Concept.** Two sides:

- **Work side.** A single action available on any surface (chat, code, cowork). The user says "send this to the AIDE queue" with a description and context. No triage, no categorisation, no thinking about which component owns it. The AI captures the item and submits it. The user stays in their work.

- **AIDE development side.** A queue that can be reviewed, triaged and worked through — individually or in batches. Triage: is it valuable, is it already covered, what component owns it, is it an extension or something new. Then the normal design-build-deploy cycle applies.

**Preferred mechanism.** An MCP server (local initially, potentially hosted/cloud later) callable from any AI surface. The MCP takes the submission and writes to a queue document in the AIDE git repo. This keeps the queue in version control alongside the framework it feeds.

**Workflow.** Submit from work → item lands in queue in git → when ready, open AIDE development project → review and triage queue → assess value, cost, coverage, ownership → allocate to component → design/build/deploy → live. Items can be worked individually or batched.

**Why this matters.** The frictionless capture path is what makes the charter's extensibility objective (O4) operational. How well this works will largely determine whether AIDE turns knowledge at the point where it is obtained into a leverageable asset, or becomes a self-serving cost.

**Component ownership.** Not yet allocated. Candidates: Infrastructure (the MCP tool and queue storage), Working Practices (the submission workflow and triage process), or a new component if the scope warrants it. To be decided when design starts.

**Cross-review required.** TBD — depends on scope of the design.
...
Version note: v4 — Submission queue concept added (frictionless capture path from work sessions to AIDE development). MCP-based mechanism, component ownership TBD. Charter reference: O4 and frictionless capture principle. 2026-09-14. Prior: v3 — DocMeth cross-review accepted. PD carries (PD-F1/F2/F3), Standards clarification carry, Core alias uniqueness carry added. Schema review thread added. FUP deployer display-order task added. 2026-09-12.
