...
> **Binder Version 9** (2026-09-02). Adds value-based dual-audience Contents/Summary semantics and the Overview escalation boundary.

This Binder is a current-context consumption artefact; authoritative masters remain individual files.

## Binder manifest

- `DocumentationMethodology_Index_v13.md` — sha256 `8998b22ed7db`
- `DocumentationMethodology_Design_v25.md` — sha256 `e26bb530ebd6`
- `DocumentationMethodology_Decisions_v26.md` — sha256 `fd385af242cb`
- `AIDE_DocumentationMethodology_Standard_v28.md` — sha256 `09c0bceb4ae3`
- `DocumentationMethodology_Guide_v28.md` — sha256 `f50ca871bc19`

---

<!-- BEGIN SOURCE: DocumentationMethodology_Index_v13.md -->
# Documentation Methodology — Index

> **Version 13** (2026-09-02). Registers Documentation Methodology v28 and the Contents/Summary/Overview orientation model.
>
...
- **Documentation Methodology** — document/corpus naming, types, lifecycle, documentation-specific
  ...
  `{standard: AIDE_DocumentationMethodology@v28}`
...
|---|---:|---|---|
| `DocumentationMethodology_Index` | v13 | Index | Current |
| `DocumentationMethodology_Design` | v25 | Design | Current confirmed internal model |
| `DocumentationMethodology_Decisions` | v26 | Decisions | Current |
| `AIDE_DocumentationMethodology_Standard` | v28 | Standard | Current; identity `AIDE_DocumentationMethodology@v28` |
| `DocumentationMethodology_Guide` | v28 | Guide | Current human/explanatory companion |

### DocumentationTopic Item Type

Defined by `AIDE_DocumentationMethodology@v28` and consumed through `AIDE_Index@v2`.
...
- v28 — `None`; DocType-defined Contents/Summary orientation and Overview escalation apply prospectively/on substantive update.
...
---
Dependencies: !AIDE_DocumentationMethodology@v28, AIDE_Index@v2, DocumentationMethodology_Design_v25
References: DocumentationMethodology_Guide_v28
<!-- END SOURCE: DocumentationMethodology_Index_v13.md -->

---

<!-- BEGIN SOURCE: DocumentationMethodology_Design_v25.md -->
...
<!-- END SOURCE: DocumentationMethodology_Design_v25.md -->

---

<!-- BEGIN SOURCE: DocumentationMethodology_Decisions_v26.md -->
# Documentation Methodology — Decisions

> **Version 26** (2026-09-02). Records the dual-audience Contents/Summary model and Overview escalation boundary.
>
> Created: 2026-08-30 | Last modified: 2026-09-02
...
## D49 — Contents is a semantic map, not a mechanical table of headings

**Decision.** Define a reusable top-of-document `Contents` section that describes the significant
information available and its stable section location. It is curated and semantic rather than
exhaustive, and exists to improve both human orientation and machine search/selective loading.

**Reason.** File-based AI contexts and human readers both benefit from understanding a document's
shape before consuming the whole body. Repeating every heading adds clutter without providing the
same semantic retrieval value.

## D50 — Summary is a faithful high-level representation controlled by DocType

**Decision.** Define a reusable `Summary` section after Contents. It gives a materially cheaper
high-level understanding of the document's objective, model/approach, key logic, boundaries and
outcome as appropriate. The detailed body remains authoritative for precision and qualification.
Each DocType owner decides applicability, role, scope and expected depth.

**Reason.** A universal fixed-size TLDR would either be too shallow for substantial Designs or add
boilerplate to documents that already expose their meaning directly.

## D51 — Orientation sections are value-based and dual-audience

**Decision.** Contents and Summary are used only when they materially improve human or machine
orientation, comprehension, discovery, search or selective loading and do not compromise the
document's function, purpose, readability or usability. Equivalent specialised structures may
satisfy the same need. Do not add either merely for structural consistency.

**Reason.** The mechanism succeeds only when its navigational/comprehension value exceeds the
clutter and maintenance burden it introduces.

## D52 — Overview remains an independent optional DocType

**Decision.** An Overview may be created by explicit user/work-owner instruction or when a
substantial high-level representation warrants its own document. It is not created only as a
mechanical overflow artefact. A domain/DocType Standard may reduce a related document's Summary
depth where a current Overview supplies richer orientation, while retaining minimum self-orientation.

**Reason.** Summary serves one document; Overview can carry a broader or more substantial
high-level model without bloating or duplicating the detailed authoritative document.

## D53 — Issue Documentation Methodology v28 prospectively

**Decision.** Publish `AIDE_DocumentationMethodology@v28` and
`DocumentationMethodology_Guide_v28` with transition posture `None`. Existing documents are not
mass-rewritten solely to add Contents or Summary. Apply the new DocType-defined posture on creation
or the next normal substantive update where it adds value.

---
Dependencies: !AIDE_DocumentationMethodology@v28, AIDE_Index@v2, DocumentationMethodology_Design_v25
References: DocumentationMethodology_Guide_v28, Core_Index_Decisions_v1, WorkingPractices_Decisions_v5, AIDE_WorkPackage@v3
<!-- END SOURCE: DocumentationMethodology_Decisions_v26.md -->

---

<!-- BEGIN SOURCE: AIDE_DocumentationMethodology_Standard_v28.md -->
# AIDE Documentation Methodology — Standard

> **Identity:** `AIDE_DocumentationMethodology@v28`
> **Common name:** Documentation Methodology
> **Version 28** (2026-09-02). Defines value-based dual-audience Contents and Summary sections and the Overview boundary.
>
> **Default weight:** Expectation

## Contents

- **Purpose and ownership** — the canonical documentation contract and boundaries with peer owners.
- **Corpus and document model** — core principles, naming, document types, condensed/expanded forms and lifecycle.
- **Live work state** — WIP, Working, OpenItems and WorkRegister admission, routing and reconciliation.
- **Document authority and history** — Design/Decisions, versioning, conformance, metadata and disposition.
- **Integration contracts** — WorkPackage, Review, Index/DocumentationTopic and distribution behaviour.
- **Navigation and semantic sections** — Binders, permitted hosts, Knowledge, Contents, Summary and Overview.

## Summary

This Standard defines how AIDE-governed document corpora name, structure, version, maintain and
distribute authoritative information while keeping volatile work, exploration, unresolved
attention, confirmed state, execution obligations and returned evidence semantically distinct.

The governing Index is the structural navigation entry point, authoritative masters remain the
source of truth, and generated Binders are current-context consumption artefacts. Documents are
hosts for owner-defined semantic sections; flexible placement does not create duplicate authority.

Where a DocType defines them, Contents and Summary provide a dual-audience top-of-document entry.
Contents maps significant information to stable locations; Summary communicates the high-level
substance. They are conditional and value-based, not universal boilerplate, and must not compromise
the document's purpose, readability or usability. Overview remains a separate optional high-level
document, created by instruction or demonstrated need.
...
`DocumentationMethodology_Guide_v28` is its fuller human-oriented companion.
...
## Top-of-document orientation

**Weight: Expectation**

`Contents` and `Summary` are reusable semantic sections, not universal required boilerplate. Their
common purpose is to reduce the cost of understanding and navigating a document for human and
machine readers without making the document less usable.

When both apply, use this order:

```text
Title / identity / essential header
Contents
Summary
Detailed body
Footer metadata
...
**Contents** is a concise curated semantic map answering what significant information the document
contains and where it is found. Describe information rather than merely repeating headings. Group
related material where useful and prefer stable heading/numbered-section locators over line numbers.
Do not attempt to enumerate every item.

**Summary** is a faithful high-level representation answering what the document establishes. Cover
the objective, overall model/approach, important logic, boundaries and outcome to the depth defined
by the DocType. It must remain substantially cheaper to consume than the whole document. It is not
independent authority: the detailed body governs where precision, qualification, evidence or
implementation detail is needed. Material inconsistency between Summary and body is a defect.

Each DocType owner defines whether Contents and Summary apply, their role/scope/depth, and whether
an equivalent specialised structure satisfies the need. Include them only where they materially
improve orientation, comprehension, discovery, search or selective loading and do not compromise
function, purpose, readability or usability. Do not add them merely for consistency.

Index, Decisions, WIP, WorkRegister, OpenItems and structured data do not use Summary by default
unless their DocType owner establishes a specific reason. Small or immediately scannable documents
may omit either section.

Overview remains an independent optional DocType. It may be created by explicit user/work-owner
instruction or because a substantial high-level representation warrants its own document. An
applicable domain/DocType Standard may reduce the Summary depth of a related document when a current
Overview provides the richer orientation, but should preserve the document's minimum self-orientation.
...
Use `AIDE_Review@v4` for the assessment lifecycle itself; this Standard governs only the document
...
---
Dependencies: AIDE_Dependencies@v3, AIDE_Migration@v3, AIDE_Tags@v2, AIDE_WorkPackage@v3, AIDE_Build@v8
References: DocumentationMethodology_Design_v25, DocumentationMethodology_Guide_v28
<!-- END SOURCE: AIDE_DocumentationMethodology_Standard_v28.md -->

---

<!-- BEGIN SOURCE: DocumentationMethodology_Guide_v28.md -->
...
<!-- END SOURCE: DocumentationMethodology_Guide_v28.md -->
