> identity: Build_Decisions@v3 | doctype: decisions | updated: 2026-09-16

# Build — Decisions

## Summary

Reasoning and resolutions from the Build design pass. Decisions D1–D12 from the voice session (2026-09-16); D13–D20 from the corpus-driven specification pass; D24–D32 from the legacy-review pass and open-question resolutions.
...
The work level is sourced from three inputs: the caller's declaration (in the build package), the profile's standing policy (for the channel), and build's own assessment (what it discovers about the work during planning and execution). The highest applicable level governs.
...
## D30. Profile storage follows the Documentation Methodology pattern
...
Build owns the mechanism; the target area owns the instance. A documentation-update profile lives under the documentation methodology area; an AIDE framework build profile lives under the AIDE area. This avoids Build becoming a dumping ground for every target domain's configuration. Same pattern as DocMeth: doctypes and block types defined centrally, instances live where the knowledge is.
...
## D32. Build standard content deferred to first real instance

The design defines what a build standard is, what it carries, and how it is authored. The seed content for the generic build standard is identified (proportionate planning, execution evidence, validation-is-not-production, out-of-scope discovery, feedback obligation). Authoring of the first build standard (the documentation-update build standard) is deferred to when that standard has a consumer — which will be the framework build.

The first instance creates the template the rest follow. Writing the standard now would invent content ahead of demonstrated need.

---

Version note: v3 — adds D24–D32 from the legacy-review pass and open-question resolutions. D24 scalability, D25 proactive feedback, D26 review dual-sourced, D27 work level, D28 cross-cutting concerns, D29 post-build dissolved, D30 profile storage, D31 autonomy default, D32 standard content deferred. 2026-09-16. Replaces v2.
