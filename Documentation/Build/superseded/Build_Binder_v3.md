...
> **Binder version 3** (2026-08-31). Rebuilt after deployment-facing Build output identity/integrity and composition-posture reconciliation.

## Binder manifest

- `Build_Index_v3.md` — sha256 `ff795af712f2`
- `Build_Design_v3.md` — sha256 `94371f077d92`
- `Build_Decisions_v3.md` — sha256 `02d18708f25e`
...
<!-- BEGIN SOURCE: Build_Index_v3.md -->
# Build — Index
...
<!-- END SOURCE: Build_Index_v3.md -->

---

<!-- BEGIN SOURCE: Build_Design_v3.md -->
# Build — Design

> **Version 3** (2026-08-31). Tightens the deployment-facing Build output contract with concrete output identity/integrity and explicit composition posture.
>
> Created: 2026-08-30 | Last modified: 2026-08-31
...
For a generated or assembled representation, Build evidence identifies the authoritative source identity/version set used sufficiently to make the result reproducible and to avoid mistaking the derived artefact for its source.

When Build output is intended for AI Deployment, the Build-to-Deployment handoff must additionally expose, directly or through the applicable representation/package contract:

- **source provenance** — the authoritative/canonical source identity/version set represented by the built material;
- **Build output identity and integrity** — enough identity and integrity evidence to identify the concrete Build result and detect substitution/change using the mechanism appropriate to that representation or package; and
- **composition posture** — whether the supplied output is a `MemberContribution` or an `AssembledConsumptionArtefact`.

The composition postures mean:

- **MemberContribution** — a target-compatible built member/contribution whose semantic content is already produced by Build and which AI Deployment may mechanically include, arrange or assemble with other built members as part of target reconciliation, without redefining its semantics.
- **AssembledConsumptionArtefact** — an authorised Build output whose semantic/member composition has already been assembled by Build. AI Deployment may deliver/reconcile that artefact but does not treat it as authority to semantically rebuild or change its member composition. A semantic/member-composition change requires another Build output.

These are required interface facts, not a mandatory universal manifest schema. An applicable platform/package contract may define their concrete encoding; otherwise the Build Outcome or equivalent handoff evidence must carry them clearly enough for downstream use.
...
AI Deployment owns target-state reconciliation, delivery/install/update/remove behaviour and verification of the resulting target/runtime state. Where the Build handoff identifies a `MemberContribution`, Deployment may perform the target-specific mechanical assembly needed for reconciliation. Where it identifies an `AssembledConsumptionArtefact`, Deployment treats the supplied semantic/member composition as Build-owned and replaces it with a later Build output when that composition must change.

Build reports only the state it actually produced and validated within its own authorised scope.
...
<!-- END SOURCE: Build_Design_v3.md -->

---

<!-- BEGIN SOURCE: Build_Decisions_v3.md -->
# Build — Decisions

> **Version 3** (2026-08-31). Adds the explicit deployment-facing Build output identity/integrity and composition-posture contract.
>
> Created: 2026-08-30 | Last modified: 2026-08-31
...
**Decision.** A Build output intended for AI Deployment exposes three interface facts: authoritative source identity/version provenance; concrete Build output/package identity and integrity evidence; and a composition posture of `MemberContribution` or `AssembledConsumptionArtefact`.

A `MemberContribution` is already semantically produced and may be mechanically assembled with other built members by Deployment. An `AssembledConsumptionArtefact` has Build-owned semantic/member composition; if that composition changes, another Build output is required rather than Deployment rebuilding the opaque artefact semantically.

**Reason.** Provenance alone identifies semantic inputs but does not identify the concrete Build result or tell Deployment what kind of composition authority remains downstream. The explicit posture prevents Deployment from inferring authority from file shape while preserving legitimate target-specific mechanical assembly.

**Alternative rejected.** Let Deployment infer whether an output is composable from filenames, payload structure or package contents. That would make the Build/Deployment boundary dependent on representation accidents and could cause an assembled Build artefact to be treated as permission for downstream semantic reconstruction.

**Implementation choice.** Do not create a universal Build manifest solely for these facts. The applicable representation/package contract may encode them; otherwise the Build Outcome or equivalent handoff evidence carries them. Build owns the semantic contract, while platform/package mechanisms may vary.

---
Dependencies: !AIDE_DocumentationMethodology@v18, Build_Design_v3
References: ProjectDesign_Design_v1, Core_System_Design_v4, AIDE_Deployment@v1
<!-- END SOURCE: Build_Decisions_v3.md -->
