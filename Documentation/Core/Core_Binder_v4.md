...
> **Binder Version 4** (2026-09-02). Adds Working Surface/Platform current design and Core Knowledge while retaining unchanged Index/Domain/Bootstrap contracts.

This Binder is a current-context consumption artefact; authoritative masters remain individual files.

## Binder manifest

- `Core_Index_v7.md` — sha256 `f9c22ac6b450`
- `Core_System_Design_v10.md` — sha256 `95a7c6b80446`
- `Core_System_Decisions_v8.md` — sha256 `17123d19e40d`
...
- `Core_Domain_Design_v4.md` — sha256 `545173606e56`
- `Core_Domain_Decisions_v4.md` — sha256 `a0365798f825`
...
- `Core_Platform_Design_v1.md` — sha256 `b92423f05e42`
- `Core_Platform_Decisions_v1.md` — sha256 `1dabe9636591`
- `Core_Knowledge_v1.md` — sha256 `3f038bf4bddb`

---

<!-- BEGIN SOURCE: Core_Index_v7.md -->
# Core — Index
...
<!-- END SOURCE: Core_Index_v7.md -->

---

<!-- BEGIN SOURCE: Core_System_Design_v10.md -->
# Core System — Design

> **Version 10** (2026-09-02). Adds the platform-agnostic Working Surface fact model and separates surface support, deployment mechanisms and actual deployed state.
>
...
Domain also owns a narrow propagation-stop boundary. `Propagation: Stop` removes the enclosing
effective Domain from the marked recognised/registered structural boundary itself and all content
within/below it. The marked boundary and its contained region then resolve independently. A parent
Index may host the Stop property on the boundary's registration without itself becoming part of the
stopped region. Independent resolution may find another Domain, `No Domain context`, or an
unresolved/error result; no parent/child Domain semantics, inheritance, merge, settings propagation
or precedence are created.

For an implicit Domain, Domain-owned metadata/settings may be hosted by an Index only when that
Index is the governing Index of an approved semantic recognised root that establishes or
participates in the Domain. Mere parent/repository registration does not confer host authority. If
settings are required, there must be one unambiguous eligible authoritative Index host; otherwise
use an explicit `AIDE_Domain.yaml` representation. Competing eligible hosts fail visibly rather
than being merged, ranked or resolved by discovery order. A native Solution/Project-only implicit
Domain therefore uses an explicit Domain representation when AIDE Domain settings are required.

Detailed semantics are owned by `Core_Domain_Design_v4`; canonical outcome:
`AIDE_Domain@v4`.
...
Canonical outcome: `AIDE_WorkingPractices@v2`.
...
Formal runtime identity: `AIDE_ProjectDesign@v5`.
...
Formal runtime identities include `AIDE_Build@v6` and `AIDE_WorkPackage@v3`.
...
Own current environment-specific configuration/state: available surface instances, channels, destinations, accounts/workspaces, model/runtime availability, access references and actual deployed/current capability state. Core Platform owns reusable generic Working Surface capability/constraint facts.
...
`AIDE_Principles@v1` and `AIDE_WorkingPractices@v2` are base guidance.
...
## §9 — Working Surfaces and platform facts

A **Working Surface** is a concrete AI/runtime surface in which AIDE-assisted work may occur. Core
owns the generic factual model used to describe a surface; it does not assign permanent design or
execution roles to products.

Record only evidence-supported facts, proportionately, including direct file read/write, repository
access, semantic/project retrieval, voice/mobile access, command execution, artefact editing,
persistent instructions, Skills/Plugins or equivalent governing-capability support, source/context
limits and supported deployment mechanisms. A surface may support several deployment mechanisms.

Keep three questions separate:

1. what the surface can support;
2. which deployment mechanisms the surface supports; and
3. which AIDE capabilities are actually deployed and current in a particular environment.

Unknown is a factual state, not evidence of support. The exact Platform Profile schema remains open
until more evidence warrants a stable canonical contract. Current evidence belongs in Core
Knowledge or another evidence owner; current environment/deployment state remains outside the
generic surface model.

Working Practices, Capability Build and AI Deployment may consume these facts without redefining
them. Security boundaries, local-filesystem reach, permissions and authority remain a recorded
future design dependency and are not solved by this model.

---
Dependencies: !AIDE_DocumentationMethodology@v23, AIDE_Index@v2
References: Core_Index_Design_v2, Core_Domain_Design_v4, Core_Bootstrap_Design_v3, Principles_Design_v3, WorkingPractices_Design_v8, ProjectDesign_Design_v5, Build_Design_v6
<!-- END SOURCE: Core_System_Design_v10.md -->

---

<!-- BEGIN SOURCE: Core_System_Decisions_v8.md -->
...
<!-- END SOURCE: Core_System_Decisions_v8.md -->
...
<!-- BEGIN SOURCE: Core_Domain_Design_v4.md -->
# Core Domain — Design
...
<!-- END SOURCE: Core_Domain_Design_v4.md -->

---

<!-- BEGIN SOURCE: Core_Domain_Decisions_v4.md -->
# Core Domain — Decisions

> **Version 4** (2026-09-01). Preserves the full Domain decision history and records the final
> Review A Round 2 determinism corrections: inclusive Propagation Stop boundary membership and
> one unambiguous eligible authoritative settings host for implicit Domains when settings are needed.
>
> Created: 2026-08-31 | Last modified: 2026-09-01

> **Current-decision note.** D1–D31 are preserved historical decisions. D32–D40 record the prior
> Review A/current lineage, and D41 onward refine the current position where stated. Historical text
> remains unchanged; later refining decisions govern the point they explicitly address.
...
## D41 — Propagation Stop includes the marked boundary

**Trigger / problem.** Review A Round 2 exposed an ambiguity in D36/D37 and the v3 contract: wording
that removed an enclosing Domain only from content `below` a Stop could leave the Domain-capable
marked boundary itself inside the enclosing Domain while requiring its contents to resolve without
that Domain. For a boundary such as `DocumentationTopic`, that makes ordinary containment/resolution
indeterminate.

**Decision.** `Propagation: Stop` takes effect at the marked recognised/registered structural
boundary itself. It removes the enclosing effective Domain from the marked boundary and all content
within/below it; that whole stopped region then resolves independently.

Where a parent Index hosts the Domain-owned Stop property on the registration for the boundary, the
parent Index is only the property host. The registered boundary is the stopped boundary; the parent
Index does not become part of the stopped region merely by hosting the property.

**Reason.** The inclusive interpretation makes Stop a coherent contextual reset. Boundary and
subtree leave the enclosing Domain together, avoiding a split state in which the boundary remains
inside one Domain while its contents are required to resolve outside it.

**Refines.** D36 and D37. Independent resolution may yield another Domain, `No Domain context`, or a
visible unresolved/error result. This does not introduce parent/child Domain semantics,
inheritance, merge, settings propagation or precedence.

## D42 — Implicit Domain settings require one unambiguous authoritative host

**Trigger / problem.** The v3 phrase `applicable authoritative host` did not determine which Index
is authoritative when several Indexes could claim Domain-owned configuration for the same implicit
Domain. Discovery order would make effective settings variable and recreate the undefined-host
problem already rejected for explicit Domains.

**Decision.** Resolve implicit-Domain settings by unique-host eligibility, not precedence.
...
- Mere parent/repository registration or location of a recognised root does not confer settings-host
  authority.
- When Domain metadata/settings are needed, exactly one eligible governing Index must be the
  unambiguous authoritative host.
- If no unique eligible Index host exists, use an explicit `AIDE_Domain.yaml` representation for
  Domain metadata/settings.
- A native Solution/Project-only implicit Domain does not acquire an arbitrary Index host merely
  because an Index registers it.
- Multiple eligible Indexes exposing/claiming Domain-owned configuration for the same implicit
  Domain fail visibly and are reconciled or made explicit; they are not merged, ranked or resolved
  by discovery order.
...
**Reason.** Uniqueness closes the host seam without inventing a generic settings precedence,
inheritance or merge engine.

**Refines.** D10, D17 and D39 for implicit-Domain host determination. D39 remains the current rule
for an explicit Domain entry: that entry is the sole authoritative Domain metadata/settings host
where it governs.

## D43 — Issue Domain v4 with migration posture None

**Decision.** Publish these final Review A corrections as `AIDE_Domain@v4` with migration posture
`None`.

**Reason.** v4 removes determinism ambiguity from current Stop and implicit-settings-host semantics
but requires no persisted consumer-state transformation merely to adopt the corrected contract.
Existing Domain representations are reconciled when substantively updated or when the v4 semantics
are applied to them.

---
Dependencies: !AIDE_DocumentationMethodology@v23, Core_Domain_Design_v4, AIDE_Index@v2
References: Core_Index_Decisions_v2, DocumentationMethodology_Decisions_v20
<!-- END SOURCE: Core_Domain_Decisions_v4.md -->
...
<!-- BEGIN SOURCE: Core_Platform_Design_v1.md -->
...
<!-- END SOURCE: Core_Platform_Design_v1.md -->

---

<!-- BEGIN SOURCE: Core_Platform_Decisions_v1.md -->

# Core Platform / Working Surface — Decisions

> **Version 1** (2026-09-02). Records the initial generic platform-fact decisions.

## D1 — Facts, mechanisms and deployed state are separate

A surface's capabilities, its supported deployment mechanisms and a particular environment's
actually deployed/current AIDE state answer different questions and remain separate.

## D2 — Evidence controls confirmation

Current product documentation may identify candidates for testing, but only evidence accepted by
the owning process becomes a confirmed AIDE platform fact.

## D3 — No broad schema yet

The initial model is an extensible factual record, not a frozen platform taxonomy.

---
Dependencies: !AIDE_DocumentationMethodology@v27
References: Core_System_Decisions_v8
<!-- END SOURCE: Core_Platform_Decisions_v1.md -->

---

<!-- BEGIN SOURCE: Core_Knowledge_v1.md -->

# Core — Knowledge

> **Version 1** (2026-09-02). Creates the Topic-level durable lateral knowledge store and preserves
> the retained OpenAI platform evidence from 2026-08-30.

## Index

- `K1` — OpenAI surface/deployment-route empirical baseline — 2026-08-30
- `K2` — Reserved platform security/filesystem-authority investigation

## K1 — OpenAI surface/deployment-route empirical baseline

**Status:** Current point-in-time evidence; re-test before treating as permanent product behaviour.

- Codex CLI/local plugin validation and explicit/implicit Skill triggering were proven, including a
  negative-trigger case. Updates were session/cache bound in the test.
- ChatGPT Cloud Work personal standalone Skill triggering was proven in Web Work and desktop Cloud
  Work. The same remote Skill updated in place and was resolved by existing/new Work conversations.
- The tested local Codex plugin did not prove ordinary Chat runtime execution and did not appear in
  ChatGPT web. The personal Cloud Work Skill was not available in ordinary Chat or Codex's local
  Skill picker.
- The proven practical split at that date was Codex CLI/local plugin; ChatGPT Cloud Work/personal
  standalone Skill; ordinary Chat/project-context Bundle fallback.
- UI/metadata presence was weaker evidence than a successful runtime probe. Representation,
  distribution channel and runtime surface must remain separate facts.

Newer possible OpenAI routes are candidates for controlled testing, not confirmed facts.

**Related:** Capabilities Capability Build; AI Deployment.

## K2 — Platform security/filesystem-authority investigation

**Status:** Reserved future investigation.

Separate technical filesystem reach from intended working scope, permissions, user authority and
security boundaries. This entry is non-executable until reconciled through the proper Design owner.

---
Dependencies: !AIDE_DocumentationMethodology@v27
References: Core_Platform_Design_v1
<!-- END SOURCE: Core_Knowledge_v1.md -->
