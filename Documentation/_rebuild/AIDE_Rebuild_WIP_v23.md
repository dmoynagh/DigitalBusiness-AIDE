# AIDE Rebuild — WIP v23

> **Version 23** (2026-09-08). **Principles masters output.** Design v4 and
> Decisions v4 authored fresh and deployed via the FileUpdatePackage pipeline
> (first real use, clean run). WIP Principles detail trimmed to a pointer —
> verified against both masters before removal, no knowledge lost.
>
> **Three outcomes folded in from Claude-Code** (BinderBuilder_Design v7 on
> Code's side): the **durability test** (what is binder-class versus
> working-class — durability, not cadence; everything that outlives the session
> is binder-class); and five **binder-settings changes** (JSON recognised,
> path-qualified exclusions, `*_Working_*` exclusion removed, empty scope writes
> empty binder, binder naming carries the binder name).
>
> **Two standing rules added.** (1) Master files are the unit of work — binders
> are a delivery convenience for context loading, not an authoring concern.
> (2) No document references binders except the binder definition itself —
> binders are a current mechanism that may change; coupling other documents to
> them creates stale references.
>
> **Note on v22.** Version 22 corrected the register/binder error, recorded the
> pending-content rule, and folded in the delivered tool pipeline. Everything in
> v22 carries forward unchanged unless an amendment note says otherwise. Its own
> summary follows.
>
> **Version 22** (2026-09-07). **The work register lives INSIDE the binder — a
> v21 error corrected by the director.** v21 recorded the register as loading
> separately from the binder as live state. That claim was carried unchanged out
> of the *old* binder's Index during sweep part 1 and never tested against the new
> model; it was then quoted back as settled. **Corrected: the register is written
> at master update, which is when the binder rebuilds anyway, so there is no churn
> to exclude it for.** The decisive argument is context cost — separate loading
> means binder plus working document plus register, multiplied by every register
> that exists, and a cluster of topics may share one.
>
> **Generalised into a rule the director then stated: the working document holds
> current working state AND pending content destined for masters not yet
> written.** Any master can have pending content — register, design, decisions,
> brief, open items. **This is not a new mechanism**: it is what capture-and-place
> already produces, now named. Pending content is held **under its destination
> document**; a master is **not authoritative alone** between updates; the working
> document is always loaded with the topic; reading a master to act on it means
> checking its pending section first. Owned whole by Working Practices.
>
> **Tool pipeline DELIVERED by Claude-Code** — binder builder change detection,
> the FileUpdatePackage deployer, and the repo configuration file, all committed
> and pushed. Two follow-on changes ratified: the rebuild folder is in binder
> scope as a **temporary** measure (the working-document exclusions are restored
> when Project Design masters land — live state stays out; the register does
> not), and **one folder may now hold several binder definitions**. **The gate on
> master-file output is discharged.**
>
> **Raised, not AIDE's:** decision identifiers now collide across corpora
> (Claude-Code's D12 and D13 are unrelated to Project Design's), so a decision
> reference crossing a message thread should name its corpus.
>
> **Note on v21.** Version 21 restated the method, completed the binder sweep and
> closed the two open items. This version corrects the register/binder error,
> records the pending-content rule, and folds in the delivered tool pipeline.
> Everything in v21 carries forward unchanged unless an amendment note says
> otherwise. Its own summary follows.
>
> **Version 21** (2026-09-07). **The rebuild method is restated, and the process
> scaffolding around it is dropped.** The governing description is now the
> director's own, in two phases and one test: **confirm what a component is for
> and how it works, until that is clear and effective on its own terms; then read
> the old material as a source and carry through only what earns its place
> against the confirmed new model.** Presence in the old version proves nothing —
> material may have evolved in rather than been chosen. Five overlapping frames
> were running at once and are collapsed: the six-stage review procedure becomes a
> silent checklist, the two-part sweep becomes one sweep, and buckets A/B/C are
> dropped entirely. The three-layer authoring model survives, because it describes
> the output rather than the process. **Design review is the load-bearing phase** —
> get purpose, objectives and methodology clear and most old-material judgements
> answer themselves, which makes the old-material pass Claude's to run rather than
> a list of calls for the director.
>
> **Binder sweep part 2 — approach — is complete**, and with it the whole sweep of
> the existing Project Design binder. Reconciled against already-settled material,
> most of part 2 turned out to be application rather than discovery. **Genuinely
> new and adopted:** the Project Design document set is design, decisions,
> standard and work register, with **no Index** (deferred to the index doctype
> review); the design document is authored in the four groups the design doctype
> advises rather than a flat numbered run; **rule weight markers** and **fenced
> pseudo-flow blocks** are lifted as techniques, the proportionate field list is
> named; and all four binder-editing items resolve as **decisions recorded against
> the new documents**, not edits to the old ones. **Withdrawn:** the
> dependency-footer lift, which contradicted the settled dependencies redesign.
> **Closed:** decisions maintenance produced no new decisions — the locked
> Decisions and Knowledge rules already settle it.
>
> **Both remaining open items are closed. Domains: AIDE does not adopt the
> concept** — topics already do the ownership job, and the genericness claim needs
> an adjective not a noun. The parked sweep finding falls away rather than
> resolving. **"A brief is the root of a design" is restatement; no change.**
>
> **Two working corrections recorded**, both from the director: orient before
> starting a task (what it is, why now, his role, what done looks like), and
> reconcile sweep findings against already-settled material *before* proposing
> them.
>
> **Note on v20.** Version 20 defined design project and build project and settled
> the binder redesign and file-output pipeline. This version restates the method,
> completes the sweep and closes the two open items. Everything in v20 carries
> forward unchanged unless an amendment note says otherwise. Its own summary
> follows.
>
> **Version 20** (2026-09-07). **Design project** and **build project** defined —
> design project is a naming convenience for an existing boundary (brief + design
> + register commitments), not a new entity; build project is deferred, the brief
> carries a plain identifier, with topic-root documentation as the recorded
> direction pending the Index doctype and the domains question. **Cross-component
> wiring** raised as a rebuild-wide item — how design, build, deployment and the
> rest are actually wired to each other has never been reviewed as a whole.
>
> **Binder redesigned from a blank sheet.** Three-layer separation: the binder
> *doctype* (what a binder is, how to read it — Documentation Methodology), the
> binder *builder tool* (how binders are created, change detection, configuration
> — Infrastructure), and *which binders exist and what they contain* (a user
> decision, outside AIDE's scope entirely). The binder exists to solve a real
> workflow problem: managing many master files and syncing them with project
> context is time-consuming, risky and a pain.
>
> **File-output approach settled.** Two surfaces, two paths: Code writes directly
> and triggers cleanup and binder rebuild; Chat/Cowork outputs a **change
> package** (a zip with files in correct paths plus a deploy manifest) for a new
> **deploy-change-package tool** that extracts, moves old versions to _superseded,
> triggers the binder builder, and waits for acknowledgement. The **tool pipeline
> is a gate on document output for the whole rebuild** — build it before
> outputting master files. Sequencing: finish the Project Design sweep, build the
> tool pipeline, output Project Design masters as first real use, then continue.
>
> **Note on v19.** Version 19 recorded part 1 of the binder sweep. This version
> records the design-project and build-project definitions, the binder redesign,
> and the file-output pipeline. Everything in v19 carries forward unchanged unless
> an amendment note says otherwise. Its own summary follows.
>
> **Version 19** (2026-09-07). **Part 1 of the `ProjectDesign_Binder_v6` sweep —
> the omissions sweep — is complete.** Reading the existing Project Design binder
> against the finished forward design produced eight candidate carries; five were
> taken, two dropped, one parked. **Carried:** build is never handed a choice
> between conflicting current designs; state the model compactly before
> elaborating it (one line of advice); handoff sufficiency has a **ceiling** as
> well as a floor; a fifth brief boundary test separating a requirement from an
> implementation choice; and a new brief property naming **which build project or
> build outcome the design is linked to**. **Dropped:** cross-topic reconciliation
> (absorbed by capture-and-place) and the no-generic-workflow-owner boundary.
> **Parked:** one-authoritative-instance-per-scope, blocked on whether AIDE uses
> the concept of *domains* at all. **Noted:** design-review instructions and
> methodology will likely reside in Project Design — revisit once Review is
> confirmed.
>
> **Three new open items**, named rather than dropped: whether AIDE uses
> **domains**; defining **design project** and **build project**, on which the new
> brief property depends; and whether "a brief is the root of a design" adds a
> hierarchy claim or merely restates what is settled.
>
> **Note on v18.** Version 18 closed the forward design pass. This version records
> part 1 of the binder sweep. Everything in v18 carries forward unchanged unless
> an amendment note says otherwise. Its own summary follows.
>
> **Version 18** (2026-09-07). **The Project Design forward design pass is
> complete**, to the limit of the Standards block — all six requirements
> delivered. Settled this pass: **capture-and-place**, reframed from a
> workflow-entity question into what it actually is — a *filing obligation*, so
> the director can stay focused on the thinking while the AI places everything
> with no valuable knowledge lost (three obligations on the AI, none on him;
> homeless pieces named not dropped; err toward over-capture; the three
> session-transition commands as batch triggers; placement defers to each
> destination's own standard for content). And the **commitment-and-return
> loop**: the escalation boundary as a single what/why-versus-how test with
> return-if-unsure; the **cost-and-complexity flag** — an obligation not a
> threshold, a flag with default proceed, deliberately unshaped pending real
> behaviour — which discharges the long-owed cost-discovery item;
> **reconciliation** defined as one design act with three endings; **return
> sufficiency** as the mirror of handoff sufficiency (never bare, attributes
> design-side versus build-side, proportional to task weight, carries review
> results); **partial coverage dissolved** by writing register items as logical
> blocks of work; **area** as a flat optional label; and **no convergence
> mechanism**, with repeat cycles noted only as a design smell worth seeing.
>
> **Two corrections worth carrying.** Claude's opening framing of
> capture-and-place — as an act on the work item, with an ownership split and a
> type list to surface — was wrong and the director dropped it; the expected work
> item type list is struck with it. And a new **Standards-wide** principle, the
> director's own generalisation: what goes into a standard is decided by "is this
> needed at the moment of application", not "is this the definition of the thing".
>
> **Note on v17.** Version 17 wrote the doctype definitions. This version closes
> the pass. Everything in v17 carries forward unchanged unless an amendment note
> says otherwise. Its own summary follows.
>
> **Version 17** (2026-09-07). Project Design design review — the doctype
> definitions are now actually written, not just catalogued, and capture-and-place is
> genuinely next. Settled this pass: the **design doctype** defined as criteria plus
> advice rather than a fixed schema (eight required properties, an advice set, and a
> one-time coverage check replacing per-element traceability); the **work register**
> defined (fields, four states, mapping to the five build-return states, and the
> handoff as the immutability boundary); the **Overview** definition split into
> required and advice; a **definition of done for component project documentation**
> as a general three-part gate; **definition of done promoted to a generic block
> owned by Working Practices**, carrying only the testable-or-assessable invariant
> while consumers fill its content; the **work item** as the base workflow entity,
> also owned by Working Practices, which **retires the design inbox** as a named
> thing; **work item and work register confirmed distinct in kind**; and the
> **ownership-designation rule** into Documentation Methodology — defining any
> doctype or block type must name its owner and residence.
>
> **Facilitate, not constrain.** Recovered from the 3 September session and now doing
> real work rather than sitting inert: AIDE exists to facilitate and empower, not to
> constrain or be a source of friction. It changed a decision this pass — the
> superseded-after-handoff rule was rewritten from a fixed procedure into a design
> judgement. Its own form and home are deferred to Core.
>
> **Note on v16.** Version 16 catalogued what the Project Design pass would produce.
> This version produces it. Everything in v16 carries forward unchanged unless an
> amendment note says otherwise.
>
> **Note on v15.** Version 15 was written to disk before its batch had been agreed,
> which is the thing the new no-output-until-agreed standing rule now prevents. Its
> content has since been confirmed in full, so it stands as an interim version rather
> than a discarded one. Carried into this version unchanged: the two remaining brief
> boundary tests (requirements vs scope; target/outcome vs definition of done); the
> design-document definition (authoritative point-in-time snapshot, sufficient alone,
> produces the tools-and-standards infrastructure, governs on conflict); the
> decisions/knowledge routing into the design workflow as a producer obligation; and
> the rule that Design carries its own live "why" inline (duplication with Decisions
> accepted) with every design-element change a retention checkpoint against Decisions.
> Carries forward from v14: the component-ownership boundary reworked
> (work register comes home to Project Design; WIP, open items, decisions and
> knowledge stay generic) with the three placement bands; the block-level portability
> convention (promote blocks, not doctypes); the forward-design amendment to review
> stages 4 and 6 (author forward first, then sweep the old corpus for omissions and
> approach); the six Project Design requirements; the design shape (one flow, three
> holding places, one seam); the element/destination map; the brief's seven-section
> specification; the work-package doctype withdrawn in favour of the design-build
> handoff at a responsibility bridgehead; build return made transactional by default;
> and the design inbox as a single inbound entry point. Carries forward from v13: the
> six-stage review procedure, elasticity and the split test, definition of done at
> principle level, and Overview with its Summary suppression rule.
...
- **Item 8 progress** — per-component design review: the three-layer authoring model, Principles (**masters output 2026-09-08** — Design v4 and Decisions v4 deployed), Project Design (forward design pass complete; **the binder sweep complete end to end**; design project and build project defined, binder redesigned, file-output pipeline settled; **only the Standards-blocked stage 6 remains**).
...
### Tool pipeline gate (raised 2026-09-07)

The tool pipeline — binder builder extension, deploy-change-package tool,
change-package format, repo configuration file — must be **complete before
outputting master files** for any component, because managing multiple file
outputs without it is too much. Sequencing: finish the Project Design sweep, then
build the pipeline (all Code tasks), then output Project Design masters as the
first real use, then continue the rebuild.

**DISCHARGED 2026-09-07 — delivered by Claude-Code** (message
`tool-pipeline/claude-code/001`). All three deliverables committed and pushed:
binder builder **change detection** (compares this run's digests against the
current binder's manifest, keeps no sidecar state, always rebuilds when it cannot
establish a baseline); the **FileUpdatePackage deployer** (package paths treated
as untrusted input, eleven scenarios verified); and the **repo configuration
file**. The misnamed-folder check was implemented as advisory only, restricted to
underscore folders after a false positive.

**One deviation accepted:** the specified `documentation_root` of `"~/"` was
rejected as circular — `"~/"` means *measured from the documentation root*, so it
cannot define that root. Shipped default is `".."`. Correct, no action.

**Two follow-on changes ratified 2026-09-07:**

1. **The rebuild folder is in binder scope — as a temporary measure.** Needed
   because Project Design and Documentation Methodology hold no masters yet, so
   the binder would otherwise be empty. **The standing position is unchanged: the
   working document is live state and stays out of the binder.** The exclusions
   are restored **when Project Design masters land** — a checkable trigger, not
   an intention. *(Note: this exclusion covers the working document only. The
   work register is inside the binder — see the correction in the Project Design
   section.)*
2. **One folder may hold several binder definitions, built in one run.** Ratified.
   The original objection was to the settings schema growing a list-of-binders
   dimension, and that objection is untouched — one file is still exactly one
   binder. What moved is the unit of *everything here* from file to folder, which
   is a different question. Change detection is what makes it worth having: before
   it, four definitions meant four writes and four version numbers every run.

**Raised back, not AIDE's to settle:** decision identifiers now **collide across
corpora** — Claude-Code's D12 and D13 are unrelated to Project Design's D12 and
D13. A decision reference crossing a message thread should name its corpus
("Claude-Code D12"). A Workflow question. **Moderate-to-strong.**

**Housekeeping owed:** the current binder carries both v21 and v22 of this
document; running version cleanup before the next binder build resolves it.
...
Reviewed under the six-stage procedure below. Stages 1, 2 and 3 complete;
stages 4, 5 and 6 outstanding.
...
#### Design doctype — criteria and advice, not a schema (settled 2026-09-07)
...
**Status (v23): PRINCIPLES MASTERS OUTPUT — Design v4 and Decisions v4 authored fresh and deployed via the FileUpdatePackage pipeline (first real use, clean run). WIP Principles detail trimmed to pointer, verified against masters before removal. Three Claude-Code outcomes folded in: the durability test (binder-class versus working-class), five binder-settings changes, both from BinderBuilder_Design v7. Two standing rules added: master files are the unit of work (binders are a delivery convenience); no document references binders except the binder definition itself. Next: Documentation Methodology design pass, then Doc Methodology masters output, then Project Design masters output. Standards-blocked stage 6 remains open for Project Design.**

**Prior status (v22): the work register lives INSIDE the binder — a v21 error corrected by the director, whose provenance was an untested confirm-and-carry from the old binder's Index. Generalised into the PENDING CONTENT rule: the working document holds current working state plus pending content destined for masters not yet written, held under its destination document; a master is not authoritative alone between updates; the working document is always loaded with the topic; reading a master to act on it means checking its pending section first. Not a new mechanism — it is what capture-and-place already produces. Owned whole by Working Practices (ownership flagged for revisit when Working Practices is worked). THE TOOL PIPELINE IS DELIVERED and the gate on master-file output is DISCHARGED — change detection, deployer and repo config all pushed; two follow-on changes ratified (rebuild folder in binder scope temporarily, restored when Project Design masters land; several binder definitions per folder). Project Design still has only stage 6 outstanding, blocked on Standards, plus the register's non-design-work question. Next: output Project Design masters as the pipeline's first real use.**

**Prior status (v21): the rebuild method is RESTATED (two phases, one test — confirm purpose and approach until clear and effective, then carry from the old material only what earns its place against the confirmed model) and the five overlapping process frames are COLLAPSED (six stages to a silent checklist, two-part sweep to one sweep, buckets A/B/C dropped, three-layer authoring model kept). Design review is the load-bearing phase; the old-material pass is Claude's to run. Two working corrections added as standing rules: orient before starting a task, and reconcile against settled material before proposing. THE BINDER SWEEP IS COMPLETE END TO END — part 2 (approach) closed, plus both open items: domains (AIDE does not adopt the concept; the parked finding falls away; the claim survives but the word does not) and brief-as-root (restatement, no change). Adopted: document set without an Index (deferred to the index doctype review); four-group design document; rule weight markers and fenced pseudo-flow blocks lifted; proportionate field list named; all four binder-editing items resolved as decisions against the new documents. Withdrawn: the dependency-footer lift. Decisions maintenance produced no new decisions — the locked rules already settle it. Only stage 6 remains for Project Design, blocked on Standards; the register's non-design-work question stays open. Next: build the tool pipeline, then output Project Design masters.**

**Prior status (v20): forward design pass COMPLETE, binder sweep part 1 (omissions) COMPLETE, design project and build project defined, binder redesigned (three-layer separation: doctype to Doc Methodology, builder to Infrastructure, configuration to user), file-output approach settled (Code-direct preferred, change package as fallback), tool pipeline is a gate on all document output. Two open items remain from part 1 (domains; brief-as-root). Next: binder sweep part 2 (approach) plus the four binder-editing items. Then build the tool pipeline before any master-file output. Stage 6 stays blocked on Standards.**

**Prior status (v19): forward design pass COMPLETE, and binder sweep part 1 — omissions — COMPLETE. Five carries taken into the design (no conflicting designs at handoff; state the model compactly; handoff sufficiency ceiling; requirement-versus-implementation as a fifth brief boundary test; the brief names its linked build project or outcome), two findings dropped, one parked on the undecided *domains* question, one noted for revisit after Review. Bucket B inventories everything the binder must give up, including all four binder-editing items, none of which is worked yet. Three homeless items named: domains; design project and build project; and whether a brief is the root of a design. Next: define design project and build project, then binder sweep part 2 (approach). Stage 6 stays blocked on Standards — the sweep completing does NOT make Project Design finished.**

**Prior status (v18): the Project Design forward design pass is COMPLETE, to the limit of the Standards block. All six requirements delivered. Doctype and block-type definitions written — brief sections, design (criteria and advice), overview, work register (fields, states, return mapping, handoff immutability, logical-block writing rule, area label), handoff, build return — plus the two generics promoted out (work item and definition of done, both to Working Practices) and the ownership-designation rule to Documentation Methodology. The design inbox is retired. This pass added capture-and-place as a filing obligation and the full commitment-and-return loop (escalation boundary, cost-and-complexity flag, reconciliation, return sufficiency). Next: the two-part binder sweep, and stage 6 when Standards unblocks it.**
