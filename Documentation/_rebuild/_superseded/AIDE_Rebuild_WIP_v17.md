...
- **Item 8 progress** — per-component design review: the six-stage procedure, three-layer authoring model, Principles (complete), Project Design (doctype definitions being finished before capture-and-place).
...
### Project Design — reviewed 2026-09-07 (stages 1–3 complete, design pass in progress)
...
**Entry — advice fields:** handoff reference, return reference.
...
#### Owed — Project Design, before the design pass can close

- **Capture-and-place mechanism** — the next item of work, and genuinely
  unblocked now: the doctypes and block types are defined, not merely named, and
  each definition carries its owner and residence. To be run in a fresh chat.
  Expected to surface the **work item type list** as an output.
- The **commitment-and-return loop**. (Brief-to-design delivery and elasticity
  now settled — see the scale-dependent brief home above.)
- **Stage 6** (design output and standards) — blocked on Standards.
- **The two-part sweep of `ProjectDesign_Binder_v6`** (omissions, then approach)
  once the forward design is complete.
- Reword D12 and bring **work-register ownership home to Project Design** in the
  binder; retire the work-package doctype (§7, `AIDE_WorkPackage@v3`) in favour
  of the design-build handoff.
- Examine binder §11–§13 (design contributions many-to-many, semantic-section
  ...
  orientation).
- Register grouping into areas as a register concept. **Moderate** — no longer
  blocked; the register doctype is settled and lives in Project Design, not
  Working Practices.
- Rename any surviving build-side **work package** to **build package** in the
  binder sweep, so it cannot collide with work item. **Strong.**
- **Interim-placement rule:** where there is nowhere proper to record something,
  place it sensibly for now and raise a review task against Project Design.
  **Strong**, new.
- Sharpen §9 into an explicit cost-discovery loop. **Moderate.**
...
**Status: doctype and block-type definitions complete and written — brief sections, design (criteria and advice), overview, work register (fields, states, return mapping, handoff immutability), handoff, build return, plus the two generics promoted out (work item and definition of done, both to Working Practices) and the ownership-designation rule to Documentation Methodology. The design inbox is retired. Next: capture-and-place in a fresh chat, then stage 6 (blocked on Standards).**
