...
> **Version 7** (2026-09-06). Settles the Dependencies block — field form, contents, header
> placement and delimiter — closing the last item §11a change management left open, and lands the
> publish gate. Dependencies moves from parked to closed in §9. Working document — does not conform
...
1. **Author the migration-format standard**, carrying R22 (cross-standard conditions) and R10
   (preserve unrelated content) as hard inputs. Authored with the doctype set, not before it.
...
- Change management: the migration-format standard is not yet authored; R22 (cross-standard
  conditions) and R10 (preserve unrelated content) are hard inputs to it. Authoring waits for the
  doctype set, per the standing sequence. (§11a)
...
4. Carry the migration-format standard (§11a) into the standards-authoring pass with R22 and R10 as
   hard inputs; land the R6 publish gate with the publish operation.
