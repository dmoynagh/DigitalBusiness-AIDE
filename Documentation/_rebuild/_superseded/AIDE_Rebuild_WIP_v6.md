...
> **Version 6** (2026-09-06). Closes §11a change management end to end: definition, detection and
> execution all settled, cross-standard dependencies resolved, rejected options recorded, the
> requirements schedule scored, and a thirty-second statement of the model. Working document —
...
**CM-Q2 — what was the document last brought into line with?** *Settled in principle.* The document
must carry this; nothing else remembers. **The stamp must be machine-findable** — fixed position,
predictable shape — so that a named-scope operation or any later wide scan can read it without
parsing whole documents. Exact form (one stamp per standard, or one combined) is the last detail
outstanding and falls out of the document structure work, not from this model.
...
override is invisible and outlives its reason.
...
- **R22.** The condition vocabulary must express conditions about another standard's behaviour, not
  only about a doctype or block type — e.g. *if your doctype extends this block type's X field, do
  Y*. Cross-standard coordination lives in migration instructions, because the standard author is
  the only party who knows what needs coordinating. **This is a hard input to the migration-format
  standard when it is authored**; a vocabulary limited to doctype and block-type conditions cannot
  carry the cross-standard case settled above.
...
| R5 | Owner writes fix, executor dumb | pass |
| R6 | No publish without a migration record | pass — enforceable as a publish gate |
| R7 | Held beside the definition | pass |
| R8 | History accumulates, prunable later | pass — pruning deferred, option preserved by CM-Q2 |
| R10 | Preserve unrelated content; fail visibly | **amber** — a rule the executor must follow, not one anything enforces |
...
| R21 | Standard behind its dependencies is detectable | **amber** — depends on the dependent standard's author writing the task; discipline, not mechanism |
| R22 | Cross-standard condition vocabulary | pass as a requirement; **hard input to the migration-format standard** |
...
**The two ambers are both real and neither is worth a mechanism now.** R10 is enforced by the
migration-format standard and by the executor's own instructions; R21 by the standard author's
diligence, backed by the fact that a missed coordination surfaces as an ordinary migration need
later rather than as silent corruption.
...
1. **CM-Q2 form** — one conformance stamp per standard, or one combined. Falls out of the document
   structure work.
2. **Author the migration-format standard**, carrying R22 (cross-standard conditions) and R10
   (preserve unrelated content) as hard inputs.
3. Confirm the publish gate for R6 lands wherever publishing is defined.
...
- Change management CM-Q2 form: one conformance stamp per standard, or one combined? Falls out of
  the document structure work. (§11a)
- Change management: the migration-format standard is not yet authored; R22 (cross-standard
  conditions) and R10 (preserve unrelated content) are hard inputs to it. (§11a)
...
4. **Author the migration-format standard** (§11a) carrying R22 and R10 as hard inputs, and land
   the publish gate that R6 requires. First substantive item for the next desktop session.
5. Block-catalogue filter — Tags / References remain parked pending their consuming components;
   Dependencies resolves out of change management (§11a) once the stamp form is settled.
