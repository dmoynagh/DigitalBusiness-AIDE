...
- **Item 8 progress** — per-component design review: the six-stage procedure, three-layer authoring model, Principles (complete), Project Design (**forward design pass complete** — doctypes, capture-and-place, and the commitment-and-return loop; only the binder sweep and the Standards-blocked stage 6 remain).
...
The brief scales by which sections are present and how deep each runs — the
block-level scaling rule doing its work inside the brief itself.
...
- **The two-part sweep of `ProjectDesign_Binder_v6`** (the existing Project
  Design binder: omissions first, then approach) now that the forward design is
  complete. Four binder-editing items ride inside this sweep and are **not**
  design work:
