...
> **Version 16** (2026-09-07). Project Design design review — doctype definitions
> finished; capture-and-place is next. Settled this pass: **design is the default** —
> a design almost always exists and a standard almost always has one behind it, with
> the no-design case an exception that must justify itself; the **doctype and
> block-type catalogue** this design work will produce; and the **brief as a composite
> block whose home is scale-dependent** — inline in the design, or split to its own
> doctype under the split test.
>
...
#### Design inbox — the inbound entry point (settled, moderate-to-strong)
...
with the block-level portability convention.

**Owned mechanisms, deliberately not fixed doctypes:** the design-build handoff,
the build return, the design inbox.
...
return** are owned transition mechanisms rather than fixed doctypes; the
**design inbox** is an owned entry point pending a portability check.
**Consumed generics:** decisions, knowledge, WIP, open items.

#### Owed — Project Design, before the design pass can close

- **Capture-and-place mechanism** — now unblocked, since the destinations are
  defined. Next item of work.
...
- Examine binder §11–§13 (design contributions many-to-many, semantic-section
  ...
  orientation).
- Register grouping into areas as a register concept. **Moderate** — pending the
  register doctype settling in Working Practices.
...
**Status: doctype definitions complete (brief sections, design, overview, work register, handoff, build return, design inbox). Next: capture-and-place, then stage 6 (blocked on Standards).**
