...
> **Version 8** (2026-09-06). Architecture review of the §11a change-management model and the
> changes arising: the three-part migration task with success checks, the renamed
> no-standard-specific-knowledge principle and the action-as-specification clause, migration
> confirmed as an AI task on every surface, and doctype modification of included blocks disallowed
> in §8 — which shrinks two requirements to near-nothing. Working document — does not conform to the
> full documentation methodology by design.
