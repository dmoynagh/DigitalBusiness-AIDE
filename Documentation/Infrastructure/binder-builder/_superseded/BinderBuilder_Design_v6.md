...
> **Version 6** (2026-09-08). Corrects §4's statement of what counts as live state: **work
> registers are inside the binder**, not loaded separately. The correction is the corpus owner's,
> relayed in `tool-pipeline/aide-rebuild-chat/002`, and is recorded here because v5 named registers
> among the documents a binder excludes and would otherwise keep teaching that. No code change, no
> behaviour change; the tool has never held an opinion about which names are live state.
>
> v5 (2026-09-07) added several binder definitions per folder — see §4b and D13.
...
settings file can be read against it:

| Class | In the binder? |
|---|---|
| **The working document** — work in progress, working notes | **No.** Loaded separately when active state is needed. |
| **Work registers** | **Yes.** A register is written at master update, which is when the binder rebuilds anyway, so the two are already in step and there is no churn to exclude it for. Loading one separately costs binder *plus* working document *plus* register, for every register that exists. |

Corrected in v6 from the corpus owner (`tool-pipeline/aide-rebuild-chat/002`, 2026-09-07); v5 and
earlier listed registers among the excluded, which was wrong. Items accumulate in the working
document between master updates and move to the register when masters update, so a reader wanting
current register items checks the working document as well as the register.
