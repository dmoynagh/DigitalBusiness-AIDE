...
> **Version 8** (2026-09-08). `exclude_files` now takes the same three path forms as `include` and
> `exclude`, so an exclusion can name where a file is and not only what it is called. Adds §4c, the
> scope resolution order as five layers, requested in `binder-settings/aide-rebuild-chat/001`. Adds
> a `_comment_defaults` key to the shipped settings so the invisible default skips are visible to a
> reader of the settings file. Existing settings files are unaffected: an entry with no `/` in it
