...
> **Version 5** (2026-09-07). One instance folder can now hold several binder definitions, and one
> run keeps them all current. Reverses D1's "one binder means one instance folder" — see D13, which
> records why the original reasoning was right and what changed underneath it. No change to the
> output format, the settings schema, or the meaning of any report word.
