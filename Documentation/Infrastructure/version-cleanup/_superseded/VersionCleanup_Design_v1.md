...
> **Version 1** (2026-09-04). Defines the version cleanup tool: matching rule, folder scope,
> archive behaviour, settings and log contract, master/instance deployment model.
...
It is a **single-action tool**, not a framework. Sibling tools (binder assembly is next) will be
separate scripts run in sequence. There is deliberately no action registry, plugin system or
shared base class. Any commonality between siblings is resolved when a second tool exists and the
duplication is visible, not in anticipation of it.
...
Afterwards the live tree carries exactly one version of each document.
...
- `~` means the **root of the tree**, never the home folder. `expanduser` is never called on
  these settings, so `~/` cannot quietly resolve to a user profile directory.
...
script**, never against the current working directory. The working directory varies
with how the script was launched (double-click, terminal, scheduler) and is unreliable; the script
folder does not. This is also what gives each instance its own settings and its own log without
...
Exercised against constructed trees covering:
...
- **Scope** - nested folders; default underscore skip; all three include forms (a relative pattern
  catching three `_binder` folders at different depths, root-anchored catching only the top one,
