...
- `_rebuild/AIDE_Rebuild_WIP_v20.md` - sha256 `8096c7a1bea8`
- `_rebuild/AIDE_Rebuild_WIP_v21.md` - sha256 `de864a9a06bf`

---

<!-- BEGIN SOURCE: _rebuild/AIDE_Rebuild_WIP_v20.md -->
# AIDE Rebuild — WIP
...
- **Item 8 progress** — per-component design review: the six-stage procedure, three-layer authoring model, Principles (complete), Project Design (forward design pass complete; binder sweep part 1 — omissions — complete; **design project and build project defined, binder redesigned, file-output pipeline settled**; part 2 (approach), the four binder-editing items and the Standards-blocked stage 6 remain).
...
already re-adopted as the register state *returned-pending-reconciliation*; the
old Standard's "apply proportionately" and "keep the model simple"; and the
Index's binder boundary — the live register loads separately from the stable
binder — which matches WIP-as-current-memory, binder-as-persisted-memory.
...
**Remaining — all parked by the director's instruction, not reopened:**

- **Stage 6** (design output and standards) — blocked on Standards.
- **Binder sweep part 2 — approach.** Part 1 (omissions) is **complete**; see the
  sweep section above. Part 2 asks whether the binder's structure and framing
  still fit the design it now has to express.
- **Two open items from part 1 not yet worked:** whether AIDE uses the concept of
  **domains** (blocking one parked sweep finding); and whether "a brief is the
  root of a design" adds a hierarchy claim or restates settled material. *(Design
  project and build project have been worked — see above.)*
...
- **The four binder-editing items**, inventoried by part 1 but not yet worked.
...
<!-- END SOURCE: _rebuild/AIDE_Rebuild_WIP_v20.md -->

---

<!-- BEGIN SOURCE: _rebuild/AIDE_Rebuild_WIP_v21.md -->
# AIDE Rebuild — WIP
...
- The work register loading separately from the binder — already carried
  unchanged from part 1, and consistent with WIP-as-current-memory,
  binder-as-persisted-memory.
...
1. **The Project Design document set is design, decisions, standard and work
   ...
   doctype anyway. **Work register is a document in the set**, excluded from the
   binder as live state. **Brief and overview are conditional**, governed by the
   split test. **Knowledge is created on demand**, not pre-created empty.
...
**Prior status (v18): the Project Design forward design pass is COMPLETE, to the limit of the Standards block. All six requirements delivered. Doctype and block-type definitions written — brief sections, design (criteria and advice), overview, work register (fields, states, return mapping, handoff immutability, logical-block writing rule, area label), handoff, build return — plus the two generics promoted out (work item and definition of done, both to Working Practices) and the ownership-designation rule to Documentation Methodology. The design inbox is retired. This pass added capture-and-place as a filing obligation and the full commitment-and-return loop (escalation boundary, cost-and-complexity flag, reconciliation, return sufficiency). Next: the two-part binder sweep, and stage 6 when Standards unblocks it.**
<!-- END SOURCE: _rebuild/AIDE_Rebuild_WIP_v21.md -->
