...
> **v17** (2026-08-19). The distributable rules of this methodology: naming, document types,
> lifecycle, and the design-to-dev-handoff workflow. Adopt this document; do not adopt the
> corpus behind it.
>
> **This is the migration target.** v16 was published as a reference copy and no project migrates
> to it. If you hold v10, v14, v15 or v16, v17 is where you are going.
>
> **Derived from `DocumentationMethodology_Design.md`**, which holds the same position for
> internal use. The reasoning behind every rule here is in
> `DocumentationMethodology_Decisions.md` — not distributed, and not needed to apply these
> rules.
>
> **v17 is the first version shaped by adopting projects rather than by design work.** Six
> containers reported their state and their findings drove most of what follows. Where a rule
> below changed because a real corpus broke against it, that is why.
>
> **New in v17 — the substantive changes:**
>
> - **§16 is new and is the largest.** Output and version discipline: a version counts *outputs*,
>   not edits; nothing is produced without confirmation; and confirmed material that exists only
>   in a conversation must be written, that last rule stated as overriding the two before it.
>   This supersedes §9's change-triggered wording and §3's version-suffix gloss.
> - **§4's type list is corrected and grows to sixteen, and the count is dropped.** `Index`,
>   `OpenItems` and `WorkRegister` were established in practice and missing from a list claiming
>   closure at ten; `Overview`, `Glossary` and `Requirements` join them. If your container
>   declared any of these as a custom type, adopt the established form.
> - **§4-r is new** — retired types and what became of each. Check it before declaring a custom
>   type: a name there is a re-derivation wanting restoration, not an invention.
> - **§13 is rewritten from prescriptive to additive.** The mandatory master is replaced by an
>   optional *owner* that may be a document, process, convention, person, or the asset itself.
>   The filename-versioning prohibition is withdrawn. The automatic lifecycle triggers are
>   withdrawn. Only the file and its deploy path are mandatory.
> - **§4h is rewritten.** Custom types are fully managed and you own their semantics and
>   lifecycle. No declaration gates creation. Nothing about a custom type is a compliance failure.
> - **§8's Index section list goes from four sections to five**, the fifth being the assets
>   register, and every document-register row now carries a **management** value: `established`,
>   `custom` or `unmanaged`. **Unmanaged files no longer get a sub-table of their own.**
> - **§15 is renamed from held files to unmanaged files**, under four classes rather than three.
>   A file your own project authored and chose not to have governed is legitimate.
> - **§4-w is new** — `Working` gets its own subsection, with a prompted disposal.
> - **§3b takes a date and a label together**, not one or the other.
> - **§4g** — Review findings carry status without touching what the Review asserts.
> - **§8c** — confirmed-versus-inferred marking now handles a mixed table.
> - **§4f** — a message without its terminator is incomplete and must not be acted on.
> - **§8a** stated four fields above a table of five. It is five. Correct your reading, not your
>   Index — if you built five fields from the table, you built it right.
>
> **Migrating?** There is no migration Guide. Instructions are drafted per project against your
> reported state — ask, and they will be issued to you.
>
> **Carried from v16 — read §4 first.** The types sit in a **stated model**: `Brief` defines,
> `Design` determines delivery, `Decisions` informs the Design and nothing downstream, and
> `Guide`, `Reference`, `WorkPackage`, skills and assets are **outcomes** that deliver. The list
> was previously flat, which left every relationship between two types to be inferred — and
> inferred differently by different readers. Several rules below change as a direct result (§4a,
> §4d, §13a).
>
> **Also new in v16:** the document footer, recording what a document was written against (§3c);
> two cross-reference forms with one meaning each (§3a); `Decisions` following `Design`'s
> granularity, with reach governed by the inheritance flag (§4d, §8a); the `Message` envelope
> rewritten, with a `STATE` block, a restored `Notes` field and multi-value `Expects` (§4f); the
> withdrawn/renamed/rehomed record (§8c); `Asset` as a category outside the type list (§13);
> claimed versus verified (§14); unmanaged files (§15); and routing stated as a test rather than a
> list of destinations (§10).
>
> **New in v15** — a stated ownership boundary between this methodology and the projects
> adopting it (§1); topic nesting, with topic and type resolved against the Index (§3a); a
> fourth mandatory Index section for topic declarations (§8a) and Index delegation (§8b);
> distribution classes (§12); the internal section convention (§11); `OpenItems` as an
> established type (§4e); custom document types (§4h); the Work Package family folded to one
> archived file, retiring `WorkPackage_Implementation` (§7a); authoring principles (§10). Also
> shorter than v14 despite the additions: reasoning that had accumulated here has moved to
> Decisions, which is where it belongs.
>
> **v15's migration Guide is withdrawn** and should not be run.
...
7. **A version counts outputs, not edits — and confirmed material is never left unwritten.**
   §16, and the second half of that rule overrides everything that restrains output.
...
### 3c. The document footer

Every document carries a footer, placed last — after all content, before the Internal section
(§11) where one exists.

```markdown
---
**Depends on:** `abc_Design_v5`, `xyz_Guide_v3`
**References:** `pqr_Reference_v8`
**Methodology:** v17
```

**A custom-typed document carries one further line, and only a custom-typed document does**
(§4h). Either the definition itself:

```markdown
**Type definition:** `Playbook` — outcome. Holds a repeatable operational procedure. Nearest
established type: `Guide`. Terminal on the procedure being retired. Internal.
```

or a pointer to where the definition is held:

```markdown
**Type:** `Playbook` — custom. Defined in `ThisProject_Index`, local configuration.
```

**Where the definition is not in the file, the file states that it is a custom type and where
the definition lives.** The three permitted definition locations then behave identically from a
reader's point of view: whatever a reader holds tells them what it is, or tells them where to
find out. A document arriving alone with an unfamiliar type segment and no marker is the failure
this prevents.

The footer is the right home rather than the internal section (§11): that section's marker
states it is not a citation source and that references from it are not to be followed, and a
type definition is precisely what a reader must be able to rely on.

**Each line records what was current when this document was last updated.** That is a historical
fact and stays true; the footer does not go stale when a listed document moves, and is not swept.
It is updated when the document itself is next updated with reference to the other.

**`Depends on` and `References` are different relationships, and the distinction is the reason
for two lines:**

- **A dependency** is content this document relies on, such that a change upstream requires
  someone to reassess this document. Several per document is normal.
- **A reference** is drawn on, mentioned or contextual. A change upstream obliges nobody.

**Neither creates automatic obligation.** A version bump upstream does not require every
dependent document to be edited. What happens is a look at what names the changed document and a
judgement — impacted or not, usually not, and then nothing is touched, footer included. That
judgement is the mechanism; the footer is what makes it cheap, because it answers "what was I
looking at" without reconstruction.

**Dependencies are declared by the dependent document**, because it is the only party that knows
it came to rely on something. No reverse index is maintained: finding what to reassess is a text
search for the changed document's name, which is why the footer must name documents rather than
describe them. A hand-maintained list of dependents decays silently — the same argument §13d
makes when it refuses a separate dependency register.

**Neither is a production record.** What produced a document, and by what process, is a separate
question and mostly outside this methodology's scope.

**`Methodology` records the version this document implements.** The governing declaration is
project-level, in the Index — every document in a container is on the same version, and a change
is a migration. The footer is the per-document record of conformance, not an independent choice,
and it exists for two reasons: a migration that misses a document shows as a mismatch rather than
passing silently, and a document that travels — a Guide deployed into a consuming project, a
document moved between containers — carries its own structural context without depending on an
Index it is no longer beside.

*This is a choice with a price, not the only workable model.* Per-document declaration — each
document stating which version it implements, and being read accordingly — is legitimate.
Project-level was chosen so that only one Guide version is distributed and no reader has to
resolve which rules a given document follows. The cost is that every document in a project moves
when the project's methodology version does.

**Required on every document.** A missing footer cannot be distinguished from an unmigrated one.
Where a document genuinely depends on or references nothing, the line reads `None.` — absence has
to be sayable so it is not confused with omission.
...
### 7a. The Work Package family

**One file open, two during execution, one archived.**

- The **Implementation** folds into the Work Package at authoring time and is no longer a
  document type — it never exists as a file. One file in, carrying the Outcome's required
  contents as a specification.
- The **Outcome** remains its own document type. It is created fresh by the builder on
  completion — not round-tripped — as its own file with its own filename, and is cited by name
  in Decisions entries, Index rows and Work Register lines. It shares its Work Package's key
  during the live phase, which is a live-phase convention for pairing by filename, not an
  archival requirement.
- On archival the **Outcome folds into the Work Package**: appended verbatim as a final
  addendum section, not summarised, re-ordered or rewritten. *A paraphrase of evidence is not
  evidence.* One file archives, taking the Work Package's name and existing key, plus
  `_Archived_{date}`.
- Consolidation is a **design-side act performed after reconciliation**.
- The consolidated file records the filenames it absorbed in its internal section (§11); the
  Index's archived line carries the absorbed Outcome's original filename so old citations
  resolve without opening the file.
- **Superseded versions are not gathered in.** Only the terminal state consolidates.
- **A bundled Design copy never enters the archive** — it is a copy of a living document and is
  deleted at merge.

Direction and progress are readable from which files exist.

### 7b. Contract and Guidance

A Work Package's implementation content carries two named parts with different force:

- **`## Contract`** — normative and binding. Not optional.
- **`## Guidance`** — advisory. Omitted where there is nothing advisory to say.

**Force is declared inside the section, not only by its heading** — a builder handed a chunk of
Guidance must know from that chunk that it is advisory.

### 7c. The Outcome's structure

- **`## Review record`** — what the review that produced this Outcome asserts. Placed after the
  substantive content, before the author's proposed document updates.
- **`## Process feedback`** — `### Package authoring` and `### Workflow and tooling`. Last
  substantive section, before any internal section.

Both state "None." explicitly when empty. Scope guidance for their content is the originating
project's.

**Model identity, within the review record.** Which model produced the review:

- **As returned by the platform** — ground truth, the primary value.
- **As set in the request** — recorded where nothing is returned, or where the two differ. An
  as-set value may be an alias.
- **Both where they differ** — the difference is itself a finding.
- **Governing profile document version and level** sit alongside as context, never as
  substitutes: a profile says which rules the review was meant to follow; the model identifier
  says what did the work.
- **Absence is recorded, not omitted.**
- **Not a plural field.** Where more than one model contributed, each is recorded against what
  it assessed. The field must remain attributable.

Values are free-form; provider vocabulary is declared in the producing project's Index local
configuration.

### 7d. Bundle self-containment and dev-side organization

A bundle carries the design context the work needs, so the dev side is not required to hold the
corpus. Dev-side live-vs-completed folder organization is the dev side's own convention and is
deliberately decoupled from corpus-side archival timing.

The dev-side skill and its marker file must say the same thing; a change to one is a change to
the other in the same pass.

### 7e. Changes outside a Work Package

Any non-trivial dev-side change made outside a Work Package gets a read-only review before
merge, answering the reconciliation question explicitly with a proposed-corpus-updates list,
even where the answer is "none." Non-trivial means touching public API surface, persisted data
shapes, or behaviour a Guide documents.
...
**Depends on:** `DocumentationMethodology_Design_v13.md`
**References:** None.
**Methodology:** v17
