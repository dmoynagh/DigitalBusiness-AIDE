...
> **Binder Version 6** (2026-09-03). Makes Change Delivery application repository-rooted and repository-relative while retaining Working Practices v2.

This Binder is a current-context consumption artefact; authoritative masters remain individual files.

## Binder manifest

- `WorkingPractices_Index_v9.md` — sha256 `3d6e552fc29a`
- `WorkingPractices_Brief_v6.md` — sha256 `956fb127091c`
- `WorkingPractices_Design_v9.md` — sha256 `628a5841bf7e`
- `WorkingPractices_Decisions_v8.md` — sha256 `b9ad4ea432c2`
- `AIDE_WorkingPractices_Standard_v8.md` — sha256 `ab8d71641c48`

---

<!-- BEGIN SOURCE: WorkingPractices_Index_v9.md -->
# Working Practices — Index

> **Version 9** (2026-09-03). Registers repository-rooted Change Delivery instructions while retaining Working Practices v2.
>
> Created: 2026-08-31 | Last modified: 2026-09-03
...
- **Working Practices** — portable cross-surface collaboration and operating conventions.  
  `{standard: AIDE_WorkingPractices@v2}`
...
|---|---:|---|---|
| `WorkingPractices_Index` | v9 | Index | Current |
| `WorkingPractices_Brief` | v6 | Brief | Current |
| `WorkingPractices_Design` | v9 | Design | Current |
| `WorkingPractices_Decisions` | v8 | Decisions | Current |
| `AIDE_WorkingPractices_Standard` | v8 | Standard | Current; identity `AIDE_WorkingPractices@v2` |

### Binder boundary

One top-level Working Practices Binder; live work state is loaded separately.
...
References: WorkingPractices_Design_v9, AIDE_WorkingPractices@v2
<!-- END SOURCE: WorkingPractices_Index_v9.md -->

---

<!-- BEGIN SOURCE: WorkingPractices_Brief_v6.md -->
# Working Practices — Brief

> **Version 6** (2026-09-02). Adds capability-led Working Context, genuine-transfer Handoff and completion reconciliation outcomes.
>
...
Working Practices now selects contexts by real capability, separates sources from governing capability activation, uses Handoff only for genuine transfer boundaries, and requires durable completion reconciliation.

---
Dependencies: !AIDE_DocumentationMethodology@v27
References: WorkingPractices_Design_v8, Principles_Design_v3
<!-- END SOURCE: WorkingPractices_Brief_v6.md -->

---

<!-- BEGIN SOURCE: WorkingPractices_Design_v9.md -->
# Working Practices — Design

> **Version 9** (2026-09-03). Makes Working Context selection capability-led, narrows Handoff to genuine transfer boundaries and adds completion/transaction rules.
>
> Created: 2026-08-31 | Last modified: 2026-09-03
...
`AIDE_WorkingPractices@v2` remains the portable base.
...
The application instructions identify the target repository and application root. Every file action uses a repository-relative destination, and the package preserves that target tree. An absolute local path may be supplied as supplemental current context, but instructions must not depend on a stale absolute path or an ambiguous current working directory.

The instructions identify, as applicable:

- add/replace action and repository-relative destination;
...
## §9 — Working Context selection and readiness

A **Working Context** is the practical surface/container in which work occurs. Select it from real
capabilities, limitations and circumstances; do not permanently assign Design, intent, precision or
execution to a named product.

Before claiming a materially dependent outcome, resolve separately:

- **work/source context** — authoritative Topic Binders/files, repository roots, uploaded/external
  material and other task sources; and
- **governing capabilities** — applicable current Standards, Tools, navigation behaviour and their
  effectively activated platform representations.

Passive presence of a Standard in a corpus is not effective activation. When a required capability
is missing or unknown and detectable, surface it before completion, explain impact and offer the
safe choices: load/deploy it, change context, or proceed with an explicit limitation if safe.
Distinguish required gaps from advantageous/preferred gaps.

## §10 — Topic ownership does not constrain work location

Semantic Topic ownership determines authoritative baseline, meaning, durable destination and
reconciliation. Coordinated cross-Topic work may occur directly in one sufficiently sourced and
authorised Working Context.

Use Project Handoff for a genuine transfer boundary: overlapping active WIP, deliberate transfer or
deferment, independent response/review, missing authoritative context, or conflict/concurrency.
Different Topic ownership alone is not a Handoff trigger.

At work-unit/session completion, no temporary Working Context may remain the sole holder of
confirmed semantic state. Reconcile confirmed work to owners, route confirmed-but-undelivered work
to durable work state, and retain in WIP only genuinely active continuation.

## §11 — Direct corpus editing and transactions

Where a Working Context directly edits authoritative files, prefer versioned/transactional storage.
Git is the preferred implementation for AIDE development corpora where practical. Group coherent
multi-file semantic changes into sensible transactions/commits. Automatic commit/push behaviour is
environment configuration, not a universal semantic rule.

---
Dependencies: !AIDE_DocumentationMethodology@v27
References: WorkingPractices_Decisions_v6, Principles_Design_v3, Core_Bootstrap_Design_v2
<!-- END SOURCE: WorkingPractices_Design_v9.md -->

---

<!-- BEGIN SOURCE: WorkingPractices_Decisions_v8.md -->
# Working Practices — Decisions

> **Version 8** (2026-09-03). Records capability-led Working Context, genuine-transfer Handoff, missing-capability and completion rules.
>
> Created: 2026-08-31 | Last modified: 2026-09-03
...
## D36 — Working Context is selected from real capability

Do not assign permanent work roles to products. Select a context from evidenced capability,
limitation and circumstances, and accept/manage those limits.

## D37 — Source context and governing capability readiness are separate

Files/repositories/Binder material supply work context. Standards, Tools and navigation behaviour
must also be effectively activated through a supported mechanism where the task depends on them.

## D38 — Topic ownership does not require Handoff

One sufficiently sourced and authorised Working Context may make coordinated cross-Topic changes.
Use Project Handoff only for a real transfer boundary, including overlapping WIP, deferment,
independent response/review, missing authority/context or concurrency conflict.

## D39 — Missing material capability is surfaced before success

Required and advantageous gaps are distinguished. Unknown is not silently supported; a safe
explicit limitation is allowed only where the missing capability is not required.

## D40 — Completion reconciles temporary context

At work-unit/session completion, reconcile confirmed semantic state to its durable owner, route
confirmed undelivered work, and retain only active continuation in WIP.

## D41 — Git is preferred but commit automation is local

Prefer versioned/transactional storage and coherent commits for direct corpus editing. Exact
auto-commit/push behaviour belongs to environment configuration.

## D42 — Issue Working Practices v2

Publish `AIDE_WorkingPractices@v2`, transition posture `None`. Existing handoffs remain valid; the
release changes the future trigger from ownership difference to genuine transfer need.

## D43 — Change Delivery application is repository-rooted and repository-relative

**Trigger / problem.** A Change Delivery package can be prepared in a different working context from the user who applies it. Absolute paths can become stale after repository moves, and an unspecified current working directory makes otherwise correct filenames ambiguous.

**Decision.** Change Delivery application instructions identify the target repository and application root, give repository-relative destinations for every file action, and preserve that target tree in the package. Absolute local paths are supplemental only and must not be the sole placement instruction.

**Reason.** The repository-relative tree is portable, reviewable and deterministic while still allowing a current local root to be stated for convenience.

## D44 — Issue Standard document v8 within AIDE_WorkingPractices@v2

**Decision.** Issue `AIDE_WorkingPractices_Standard_v8.md` while retaining formal capability identity `AIDE_WorkingPractices@v2` and migration posture `None`.

**Reason.** This strengthens an existing Change Delivery practice; it does not introduce a new capability semantic release or persisted consumer-state migration.

---
Dependencies: !AIDE_DocumentationMethodology@v28, WorkingPractices_Design_v9
References: DocumentationMethodology_Decisions_v19, Principles_Decisions_v3
<!-- END SOURCE: WorkingPractices_Decisions_v8.md -->

---

<!-- BEGIN SOURCE: AIDE_WorkingPractices_Standard_v8.md -->
# AIDE Working Practices — Standard

> **Identity:** `AIDE_WorkingPractices@v2`
> **Common name:** Working Practices
> **Version 8** (2026-09-03). Makes Change Delivery application repository-rooted and repository-relative while preserving the v2 capability contract.
>
...
---
Dependencies: !AIDE_DocumentationMethodology@v27
References: WorkingPractices_Design_v9, AIDE_Principles@v1
<!-- END SOURCE: AIDE_WorkingPractices_Standard_v8.md -->
