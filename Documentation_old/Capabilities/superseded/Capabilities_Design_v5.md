...
> **Version 5** (2026-08-29). Finalises the Review component's independent-assessment model,
> lifecycle, profiles, orchestration boundary, records, scope control, and external environment /
> communication seams.
>
...
Its purpose is to improve substantive integrity, support better decisions, and obtain confidence
proportionate to consequence and uncertainty by introducing a meaningfully separate reasoning
path. The Lead owns the current model/outcome, its authorised scope, net coherence, and final
disposition. Reviewer Findings are evidence and input, not instructions.

The reusable dimensions are independent:

- **Type** — what the Review is trying to learn and the lens applied;
- **Level** — assurance strength justified by consequence, reach, reversibility, and uncertainty;
- **Mode** — Full or Blind exposure to the Lead's existing reasoning/solution; and
- **Reviewer** — the separate reasoning source selected from available environment capability.

The standard Types are Check, Inspect, Evaluate, Robust, and user-activated Stress Test. Level is
dynamic and may escalate or de-escalate as Findings change the risk picture. Higher Levels increase
evidence, independence, iteration, completion confidence, and re-review of substantive changes.

Review owns the Review Input Contract, purpose-shaped request, Review/Round/Result semantics,
Finding/disposition separation, continuation and completion discipline, and the boundary that
permits discovery beyond authorised scope without silently expanding execution.

Review owns the exchange and its state, not delivery. Environment configuration supplies current
reviewers, models, capabilities, and routes. A shared communication capability performs direct or
AI Message-based transport. The ownership/storage homes for those two external seams remain
...
Review can provide independent assessment during design exploration, pre-confirmation design,
WorkPackage authoring, Build planning, and post-execution validation. Its lifecycle is cross-cutting
and does not become another production stage.

---

## §18 — Deliberately open

- Environment settings home for current reviewer/model/capability/route availability.
- Permanent ownership and full contract of the shared communication capability consumed by Review
  and future inter-AI behaviours such as Research.
...
**Depends on:** `Capabilities_Brief` v4, `Capabilities_Decisions` v11,
`Capabilities_Review_Design` v1.
...
`AIDE_Review@v1`, `AIDE_ReviewProfiles@v1`,
