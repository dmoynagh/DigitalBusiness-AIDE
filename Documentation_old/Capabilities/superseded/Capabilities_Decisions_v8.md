...
> **Version 8** (2026-08-28). **Parent architecture reset.** Records the seven-component
> architecture and reconciles earlier decisions on ownership, platform divergence, production,
> dependency semantics, migration, On-Update behaviour, commands, Review, and DocMeth boundaries.
> Earlier decisions remain in place as design history and are marked where revised or superseded.
>
...
**Status: retained; ownership revised by `D45`.** Scope is now a first-class component and owns
the shared model and its platform realisation.
...
**References:** `Capabilities_Design` v2, `Capabilities_Standards_Design` v3 (revision required),
`Capabilities_Tools_Design` v1 (revision required), `Capabilities_Brief` v2,
